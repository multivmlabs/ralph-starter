...
Every time ralph-starter runs Claude Code in a loop, that first iteration ships everything over: system prompt, project files, the spec, the implementation plan. All fresh tokens. Expensive. But here is the thing -- Claude's prompt caching stores all of that server-side. So on loop 2, 3, 4? Those same tokens cost 90% less. You just... get a discount for repeating yourself.
...
But honestly? The biggest cost optimization is not a caching trick. It is writing [better specs](/blog/specs-are-the-new-code). A clear spec means the agent gets it right in fewer loops. Fewer loops means fewer API calls. A spec that lands in 2 iterations instead of 7 saves you way more than any caching ever could.
...
- [10 GitHub issues and went to lunch](/blog/automating-entire-workflows)
